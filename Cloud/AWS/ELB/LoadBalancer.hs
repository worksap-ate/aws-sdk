...
import Cloud.AWS.Lib.Parser.Unordered (SimpleXML, xmlParser, (.<), content, getElement, getElementM)
...
describeLoadBalancers lbs marker =
    elbQuery "DescribeLoadBalancers" params $
        xmlParser sinkLoadBalancers
  where
    params =
...
createLoadBalancer name listeners zones scheme groups subnets =
    elbQuery "CreateLoadBalancer" params $ xmlParser $ (.< "DNSName")
  where
    params =
...
deleteLoadBalancer name = elbQuery "DeleteLoadBalancer" params $
    xmlParser $ (.< "DeleteLoadBalancerResult")
  where
...
attachLoadBalancerToSubnets name subnets =
    elbQuery "AttachLoadBalancerToSubnets" params $ xmlParser $
        members' "Subnets" content
  where
    params =
        [ "LoadBalancerName" |= name
        , "Subnets.member" |.#= subnets
...
detachLoadBalancerFromSubnets name subnets =
    elbQuery "DetachLoadBalancerFromSubnets" params $ xmlParser $ 
        members' "Subnets" content
  where
    params =
        [ "LoadBalancerName" |= name
...
applySecurityGroupsToLoadBalancer name sgs =
    elbQuery "ApplySecurityGroupsToLoadBalancer" params $
        xmlParser $ members' "SecurityGroups" content
  where
    params =
        [ "LoadBalancerName" |= name
...
registerInstancesWithLoadBalancer insts name =
    elbQuery "RegisterInstancesWithLoadBalancer" params $
        xmlParser $ members' "Instances" sinkInstance
  where
    params =
...
createLoadBalancerListeners listeners lb =
    elbQuery "CreateLoadBalancerListeners" params $ xmlParser $
...
deleteLoadBalancerListeners lb ports =
    elbQuery "DeleteLoadBalancerListeners" params $ xmlParser $
...
describeLoadBalancerPolicies mlb policies =
    elbQuery "DescribeLoadBalancerPolicies" params $ xmlParser $
...
createLoadBalancerPolicy lb attrs name typeName =
    elbQuery "CreateLoadBalancerPolicy" params $ xmlParser $
...
deleteLoadBalancerPolicy lb policyName =
    elbQuery "DeleteLoadBalancerPolicy" params $ xmlParser $
...
describeInstanceHealth insts lb =
    elbQuery "DescribeInstanceHealth" params $ xmlParser $
...
configureHealthCheck hc lb =
    elbQuery "ConfigureHealthCheck" params $ xmlParser $ \xml ->
        getElement xml "HealthCheck" sinkHealthCheck
  where
    params =
...
createLBCookieStickinessPolicy period lb policy =
    elbQuery "CreateLBCookieStickinessPolicy" params $ xmlParser
...
createAppCookieStickinessPolicy cookieName lb policy =
    elbQuery "CreateAppCookieStickinessPolicy" params $ xmlParser
...
setLoadBalancerPoliciesOfListener lb port policies =
    elbQuery "SetLoadBalancerPoliciesOfListener" params
    $ xmlParser (.< "SetLoadBalancerPoliciesOfListenerResult")
  where
    params =
        [ "LoadBalancerName" |= lb
        , "LoadBalancerPort" |= port
...
setLoadBalancerPoliciesForBackendServer port lb policies =
    elbQuery "SetLoadBalancerPoliciesForBackendServer" params
    $ xmlParser (.< "SetLoadBalancerPoliciesForBackendServerResult")
  where
    params =
        [ "InstancePort" |= port
        , "LoadBalancerName" |= lb

module AWSTests.RDSTests
    ( module AWSTests.RDSTests.DBInstanceTests
    , module AWSTests.RDSTests.DBParameterGroupTests
    , module AWSTests.RDSTests.DBSecurityGroupTests
    , module AWSTests.RDSTests.DBSnapshotTests
    , module AWSTests.RDSTests.DBSubnetGroupTests
    , module AWSTests.RDSTests.EventTests
    )
    where

import AWSTests.RDSTests.DBInstanceTests
import AWSTests.RDSTests.DBParameterGroupTests
import AWSTests.RDSTests.DBSecurityGroupTests
import AWSTests.RDSTests.DBSnapshotTests
import AWSTests.RDSTests.DBSubnetGroupTests
import AWSTests.RDSTests.EventTests

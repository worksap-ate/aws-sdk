module AWSTests.EC2Tests
    ( module AWSTests.EC2Tests.InstanceTests
    , module AWSTests.EC2Tests.VPCTests
    , module AWSTests.EC2Tests.AddressTests
    , module AWSTests.EC2Tests.SnapshotTests
    , module AWSTests.EC2Tests.ImageTests
    , module AWSTests.EC2Tests.SubnetsTests
    , module AWSTests.EC2Tests.VolumeTests
    , module AWSTests.EC2Tests.SecurityGroupTests
    , module AWSTests.EC2Tests.RouteTableTests
    , module AWSTests.EC2Tests.RegionTests
    , module AWSTests.EC2Tests.AvailabilityZoneTests
    , module AWSTests.EC2Tests.TagTests
    , module AWSTests.EC2Tests.KeyPairTests
    , module AWSTests.EC2Tests.NetworkInterfaceTests
    )
    where

import AWSTests.EC2Tests.InstanceTests
import AWSTests.EC2Tests.VPCTests
import AWSTests.EC2Tests.AddressTests
import AWSTests.EC2Tests.SnapshotTests
import AWSTests.EC2Tests.ImageTests
import AWSTests.EC2Tests.SubnetsTests
import AWSTests.EC2Tests.VolumeTests
import AWSTests.EC2Tests.SecurityGroupTests
import AWSTests.EC2Tests.RouteTableTests
import AWSTests.EC2Tests.RegionTests
import AWSTests.EC2Tests.AvailabilityZoneTests
import AWSTests.EC2Tests.TagTests
import AWSTests.EC2Tests.KeyPairTests
import AWSTests.EC2Tests.NetworkInterfaceTests
